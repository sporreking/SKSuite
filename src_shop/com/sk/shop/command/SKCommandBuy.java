...
public class SKCommandBuy implements CommandExecutor {
	
	private SKShopMain main;
	
	public SKCommandBuy(SKShopMain main) {
		this.main = main;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(!player.hasPermission("sk.shop.entry.add.buy.self")) {
		...
		try {
			...
			if(!player.getName().equals(shop.getOwnerName()) && !player.hasPermission("sk.shop.entry.add.buy.others")) {
			...
			shop.addEntry(new SKShopEntry(shop, SKShopEntry.ENTRY_BUY, is.getType().name(), is.getAmount(),
					Double.parseDouble(args[1]), args.length > 2 ? Integer.parseInt(args[2]) : 0));
			
			sender.sendMessage(ChatColor.BLUE + shop.getName() + ChatColor.GREEN + " is now buying "
